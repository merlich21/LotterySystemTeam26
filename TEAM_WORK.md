...
- Скрипт тестирования API через curl
- Запросы тестирования API в Postman
...
- [ ] Создать `HibernateConfig` для настройки подключения к БД и конфигурации ORM
- [ ] Создать `SerializationConfig` для настройки сериализации/десериализации в Javalin
...
**Задачи**:
- [ ] `GeneratorUtil`: генерация чисел
- [ ] `Validator`: валидация dto
- [ ] `UserService`: регистрация, авторизация пользователей
...
**Проверка**: `infra/api.sh` проверяет работу API через curl, все сценарии в README работают без ошибок
